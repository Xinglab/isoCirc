...
# This is a script that creates a cumulative distribution plot for the transcript length
...
# Usage:
#	Rscript transcript_length_CDF.R -i /path/to/input/file \
...
create_df <- function(infile) {
	# Read in input file as data frame and select BSJCate, FSJCate, and refMapLen columns
	inDF <- read.table(infile, sep='\t', header=TRUE)
	inDF <- inDF %>% select(BSJCate, FSJCate, refMapLen)
	
	# Create separate data frame for plotting CDF
	plotDF <- data.frame(BSJCate=c(), FSJCate=c(), refMapLen=c(), group=c())
...
make_plot <- function(plotDF, outfile) {
	...
	xmin <- 10^floor(log(min(plotDF$refMapLen), 10))
	xmax <- 10^ceiling(log(max(plotDF$refMapLen), 10)) * 0.5
	ymin <- 0
	ymax <- 1
	...
	# Create plot
	p <- ggplot(plotDF, aes(refMapLen, color=group)) + 
		stat_ecdf(size=line_size) +
		xlab(xlabel) +
		ylab(ylabel) +
		scale_x_continuous(limits=c(xmin,xmax), 
			breaks=c(xmin, 100, 300, 500, 1000, xmax), 
			trans='log10', 
...
main <- function() {
	moduleSummary <- 'Creates CDF plot for transcript length per circRNA isoform.'

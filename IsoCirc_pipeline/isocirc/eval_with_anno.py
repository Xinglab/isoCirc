...
def output_isoform_eval(out_fp, all_out, itst_out_dict, iso_to_name_dict={}):
...
def circRNA_filter_core(eval_out, bsj_motifs=['GT/AG'], bsj_xnum=1, bsj_key_xnum=0, min_circ_dis=150):
	# filter #1
...
def eval_with_anno(data_type, high_bam, low_bam, long_len, cons_info, cons, 
...
def eval_with_anno_core(args):
	itst_anno_dict = dd(lambda : '')
	itst_anno_dict['Alu'] = args.Alu
	itst_anno_dict['allRepeat'] = args.all_repeat

	eval_with_anno(args.type, args.high_bam, args.low_bam, args.long_len, args.cons_info, 
